...
from simulator.event_schema import (
    BaseEvent, PageViewEvent, AddToCartEvent, PaymentEvent, UserSignupEvent
)
...
def test_events_per_minute() -> None:
...
def test_unique_pages_and_revisit_ratio() -> None:
    events: list[BaseEvent] = [
...
def test_cart_to_purchase() -> None:
    events: list[BaseEvent] = [
...
def test_session_duration_and_gaps() -> None:
    events: list[BaseEvent] = [
        PageViewEvent(event_id="1", timestamp=t0, session_id="s1", user_id="u1", url="/"),
...
def test_diversity_and_payment() -> None:
    events: list[BaseEvent] = [
        PageViewEvent(event_id="1", timestamp=t0, session_id="s1", user_id="u1", url="/"),
        PaymentEvent(event_id="2", timestamp=t0, session_id="s1", user_id="u1", amount=10.0, status="success"
                     , payment_method="credit_card")
...
def test_signup_to_purchase_speed() -> None:
    events: list[BaseEvent] = [
